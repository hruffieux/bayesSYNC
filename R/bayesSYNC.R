...
bayesSYNC_core <- function(N, p, L,Q, K, C, Y, list_hyper, time_obs, n_g,
  ...
  for(i_iter in 1:maxit) {
    ...
    for (j in 1:p){
      ...
      Sigma_q_nu_mu_j <- solve (E_q_inv_Sigma_nu_mu + mu_q_recip_sigsq_eps[j]* sum_term_Sigma_j)
      mu_q_nu_mu_j <- as.vector(Sigma_q_nu_mu_j %*%(mu_q_recip_sigsq_eps[j]*sum_term_mu_j))
      Sigma_q_nu_mu[[j]] <- unlist(Sigma_q_nu_mu_j)
      ...
      for (l in 1:L){
        ...
        for (i in 1:N){
          for (j in 1:p){
            ...
            sum_val_ql_prime <-rep(0, dim(C[[i]])[2])
            for (q_tilde in 1:Q){
              for (l_tilde in 1:L){
        ...
        Sigma_q_nu_phi_ql <- solve(sum_val_2* sum_val+ E_q_inv_Sigma_nu_phi)
        Sigma_q_nu_phi[[q]][[l]] <- Sigma_q_nu_phi_ql
        mu_q_nu_phi[[q]][[l]]<-Sigma_q_nu_phi_ql%*%sum_term_mu_ql
      }
    }
    # Update of q(zeta_iq):
    for (i in 1:N){
      ...
      for(q in 1:Q){
        ...
        for( i in 1:N){
          ...
          sum_mu <- sum_mu +t(mu_q_zeta[[i]][[q]])%*%
            t(mu_V_q_phi)%*%t(C[[i]])%*%(Y[[i]][[j]]-C[[i]]%*%mu_q_nu_mu[[j]]-C[[i]]%*%sum_mu_tilde)
        }
    ...
    ELBO_iter <- ELBO_iter+ sum(sapply(1:p, function(j){
      ...
      -1*log(sigma_zeta)+ 0.5*log_det_j-(K/2)*(mu_q_log_sigsq_mu[j])-0.5*(unlist(t(mu_q_nu_mu[[j]])%*%E_q_inv_Sigma_nu_mu%*%mu_q_nu_mu[[j]])+ tr(E_q_inv_Sigma_nu_mu %*% Sigma_q_nu_mu[[j]])) +K/2 + 1+
    ...
    ELBO_iter<- ELBO_iter+ sum(sapply(1:Q, function(q){
      sum(sapply(1:L, function(l){
        ...
        -1*log(sigma_zeta)+ 0.5*log_det_q_l-(K/2)*(mu_q_log_sigsq_phi[q,l])-
          0.5*(unlist((t(mu_q_nu_phi[[q]][[l]])%*%E_q_inv_Sigma_nu_phi%*%mu_q_nu_phi[[q]][[l]])+ tr(E_q_inv_Sigma_nu_phi%*% Sigma_q_nu_phi[[q]][[l]])))+
          K/2 + 1 #mu_phi
      }))
    }))
    ELBO_iter<- ELBO_iter + sum( sapply(1:Q, function(q){
